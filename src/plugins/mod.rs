    ...
    fn initialize(&mut self, conf_path: &str) -> Result<(), String> {
        unsafe {
            if self
                ...
                .borrow_mut()
                .cnt
                .dsvchc_plugin_check_initialize(self.chk, &PluginStr::from_str_ref(conf_path))
                != PLUGIN_ERR_CODE_OK
            {
                return Err((*self
                    .ctx
                    .borrow_mut()
                    .cnt
                    .dsvchc_plugin_check_get_error(self.chk))
                .to_string());
            }
        }
        Ok(())
    ...
    fn perform(&mut self) -> Result<PluginCheckResult, String> {
        unsafe {
            let c = self
                ...
                .borrow_mut()
                .cnt
                .dsvchc_plugin_check_perform(self.chk);
                ...
                Ok(PluginCheckResult {
                    ...
                    extra: RawValue::from_string(
                        (*self
                            ...
                            .borrow_mut()
                            .cnt
                            .dsvchc_plugin_check_get_result_json(self.chk))
                        .to_string(),
                    )
                    .unwrap(),
                })
    ...
    fn finalize(&mut self) -> Result<(), String> {
        unsafe {
            if self
                ...
                .borrow_mut()
                .cnt
                .dsvchc_plugin_check_finalize(self.chk)
                != PLUGIN_ERR_CODE_OK
            {
                return Err((*self
                    .ctx
    ...
    fn drop(&mut self) {
        unsafe {
            self.ctx
                .borrow_mut()
                .cnt
                .dsvchc_plugin_check_destroy(self.chk)
        };
    }
}
...
struct PluginApi {
    ...
    dsvchc_plugin_check_initialize:
        unsafe extern "C" fn(check: *mut c_void, conf_path: *const PluginStr) -> i32,

    dsvchc_plugin_check_perform: unsafe extern "C" fn(check: *mut c_void) -> i32,

    dsvchc_plugin_check_get_result_json:
        unsafe extern "C" fn(check: *mut c_void) -> *const PluginStr,

    dsvchc_plugin_check_finalize: unsafe extern "C" fn(check: *mut c_void) -> i32,

    dsvchc_plugin_check_get_error: unsafe extern "C" fn(check: *mut c_void) -> *const PluginStr,

    dsvchc_plugin_check_destroy: unsafe extern "C" fn(check: *mut c_void),
